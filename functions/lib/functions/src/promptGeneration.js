...
exports.generatePrompt = (0, https_1.onCall)(async (request) => {
    ...
    try {
        ...
        const prompt = await (0, privacyControls_1.withPrivacyConsent)("prompt_generation")(data.userId, async () => {
...
exports.generateBulkPrompts = (0, https_1.onCall)(async (request) => {
    ...
    try {
        ...
        const results = await (0, privacyControls_1.withPrivacyConsent)("bulk_prompt_generation")(data.userId, async () => {
            const startTime = Date.now();
            ...
            const prompts = (await Promise.all(promptPromises)).filter((p) => p !== null);
            const duration = Date.now() - startTime;
 ...
 */
async function generateSinglePrompt(data) {
    if (!genAI) {
 ...
 */
function calculateExpiryDate(urgency) {
    const now = new Date();
 ...
 */
function createTimeoutFallbackPrompt(data) {
    const { relationshipContext } = data;
...
exports.promptGeneration = {
    generatePrompt: exports.generatePrompt,
    generateBulkPrompts: exports.generateBulkPrompts,
    evaluatePromptRelevance: exports.evaluatePromptRelevance,
};
