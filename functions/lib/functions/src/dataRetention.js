...
exports.RETENTION_POLICIES = RETENTION_POLICIES;
/**
 ...
 */
async function deleteUserData(userId, requestId) {
    ...
    // Log the completion
    await logPrivacyAction(userId, 'data_deletion', 'data_collection', `Complete user data deletion completed. ${deleteCount} documents deleted.`, 'system');
}
...
async function generateUserDataExport(userId) {
    ...
    exportData.relationships = await Promise.all(relationships.docs.map(async (relationshipDoc) => {
    ...
    const twelveMonthsAgo = admin.firestore.Timestamp.fromDate(new Date(Date.now() - 12 * 30 * 24 * 60 * 60 * 1000));
...
exports.generateUserDataExport = generateUserDataExport;
/**
 * Create download URL for export data
 * In production, this would upload to Cloud Storage
 */
async function createExportDownload(userId, exportData) {
...
async function cleanupInactiveUsers() {
    const cutoffDate = admin.firestore.Timestamp.fromDate(new Date(Date.now() - RETENTION_POLICIES.INACTIVE_USER_MONTHS * 30 * 24 * 60 * 60 * 1000));
...
async function cleanupOldAuditLogs() {
    const cutoffDate = admin.firestore.Timestamp.fromDate(new Date(Date.now() - RETENTION_POLICIES.AUDIT_LOG_MONTHS * 30 * 24 * 60 * 60 * 1000));
 ...
 */
async function cleanupOldConsentHistory() {
    const cutoffDate = admin.firestore.Timestamp.fromDate(new Date(Date.now() - RETENTION_POLICIES.CONSENT_HISTORY_YEARS * 365 * 24 * 60 * 60 * 1000));
    let cleanedCount = 0;
    // Get all users (in batches)
 ...
 */
async function logPrivacyAction(userId, action, permissionUsed, details, performedBy) {
    try {
 ...
 */
exports.dataRetentionHealthCheck = (0, https_1.onRequest)(async (req, res) => {
    try {
