...
exports.detectRelationshipConflicts = (0, firestore_1.onDocumentWritten)('users/{userId}/relationships/{relationshipId}', async (event) => {
    ...
    try {
        ...
        const conflict = await detectConcurrentModification(userId, relationshipId, oldData, newData, change.after.updateTime);
        if (conflict) {
...
exports.detectInteractionConflicts = (0, firestore_1.onDocumentWritten)('users/{userId}/relationships/{relationshipId}/interactions/{interactionId}', async (event) => {
    ...
    try {
        ...
        const conflict = await detectInteractionMergeConflict(userId, relationshipId, interactionId, oldData, newData);
        if (conflict) {
            await createConflictRecord(userId, conflict);
 ...
 */
async function detectConcurrentModification(userId, relationshipId, oldData, newData, updateTime) {
 ...
 */
function detectConflictingFields(oldData, newData) {
 ...
 */
async function createConflictRecord(userId, conflict) {
 ...
 */
async function notifyUserOfConflict(userId, conflict) {
 ...
 */
async function attemptAutomaticResolution(userId, conflict) {
    ...
    switch (conflict.conflictType) {
        ...
        case 'relationship_health_conflict':
            ...
            resolvedData = {
                ...
                relationshipHealth: Math.max(conflict.localVersion.relationshipHealth || 0, conflict.serverVersion.relationshipHealth || 0)
            };
            break;
        default:
 ...
 */
async function performIntelligentMerge(conflict) {
    ...
    switch (conflict.conflictType) {
        case 'relationship_data_conflict':
            ...
            merged.relationshipHealth = Math.max(conflict.localVersion.relationshipHealth || 0, conflict.serverVersion.relationshipHealth || 0);
            break;
        default:
 ...
 */
async function mergeInteractions(conflict) {
 ...
 */
async function applyResolvedData(userId, documentPath, resolvedData) {
 ...
 */
async function createResolutionAuditRecord(userId, conflictId, resolution, resolvedBy) {
